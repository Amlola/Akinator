...
void StackCtor(Stack* stk)
    {
    if (logfile_stack == NULL)
        {
        logfile_stack = fopen("logfile_stack.html", "w");
        }
    fprintf(logfile_stack, "<pre>\n");
...
ON_DUMP
    ...
    void StackDumpFunction(Stack* stk, const char* path, const char* signature, unsigned line)
        {
        fprintf(logfile_stack, "<font color = \"#964b00\">-----------------------------------------------------------------------\n</font>");

        fprintf(logfile_stack, "<font size = \"+1\">path: %s\n</font>", path);
        fprintf(logfile_stack, "<font size = \"+1\">in function: %s\n</font>", signature);
        fprintf(logfile_stack, "<font size = \"+1\">line: %d\n</font>", line);
        fprintf(logfile_stack, "\n");
        fprintf(logfile_stack, "pos  = %d\n", stk->stack_pos);
        fprintf(logfile_stack, "size = %d\n", stk->stack_size);
        fprintf(logfile_stack, "data[%p]\n", stk->stack_data);
        ...
        ON_HASH
            ...
            if (stk->stack_data != NULL && stk->stack_size > 0 && stk != nullptr && stk->stack_pos > 0)
                {
                fprintf(logfile_stack, "hash data  = %lld\n", stk->hash_data);
                fprintf(logfile_stack, "hash stack = %lld\n", stk->hash_stack);
                fprintf(logfile_stack, "\n");
                }
            )
        ...
        ON_CANARY
            ...
            if (stk->stack_data != NULL && stk->stack_size > 0 && stk != nullptr && stk->stack_pos > 0)
                {
                fprintf(logfile_stack, "left struct canary  = 0x%x\n", stk->LeftCanary);
                fprintf(logfile_stack, "right struct canary = 0x%x\n", stk->LeftCanary);
                fprintf(logfile_stack, "left data canary    = 0x%x\n", LEFTCANARYDATA);
                fprintf(logfile_stack, "right data canary   = 0x%x\n", RIGHTCANARYDATA);
                fprintf(logfile_stack, "\n");
                }
            )

        if (stk->stack_status != STACK_OK) 
            {
            for (size_t j = 0; j < NUMBER_OF_ERR; j++)
                {
                if ((stk->stack_status & (1 << j)))
                    {
                    fprintf(logfile_stack, "<font color = \"red\">ERROR: %s\n</font>", ErrorArray[j + 1].NameError);
                    }
                }
            }
